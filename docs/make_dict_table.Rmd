...
```{r cogresults,echo=FALSE,eval=TRUE}
...
# clustdata[,'DXSubAsyn']=factor(clustdata[,'DXSubAsyn'],levels=dxs)
...
###################################################"mclust","flexcorr",
osex='SEX'
trainsel = fs( basesel  ) # & is.na( subcohort$AsynStatus ) )
trainsel = fs( basesel & subcohort$DX %in% c( "PD" ) )#  & subcohort$SEX==0)
trainsel = fs( basesel & subcohort$DXSubAsyn %in% c('PDSporadicPositive') )
trainsel = fs( basesel & subcohort$DX %in% c( "PD" ) )#  & subcohort$SEX==0)
trainsel = fs( subcohort$DX == "PD" )
desiredk = 4
clustlist = c( "GMM", "kmeans", "pamCluster", 
    "kmeansflex","kmedians",  "angle",  "ejaccard", 
    "hardcl","neuralgas", "hierarchicalCluster" )
clustlist = c(
    "GMM", 'kmeansflex', "pamCluster", 
    "kmeansflex", "kmedians", "angle", "ejaccard", "hardcl" )
clustdf = data.frame( )
for ( myclust in clustlist ) {
    print( paste( "myclust", myclust, 'begin' ) )
    cohsel = clustdata$DX %in% c('PD','Prodromal','CN')
    subcohort = clustdata[ cohsel,]
    basesel = fs( subcohort$imaging_EVENT_ID == 'V0' )
    subcohortbl = subcohort[ basesel, ]
    mcnames = testnames[ -multigrep(c("DTI","mtl","cereb"), testnames) ]
    mcnames = c( testnames[ -multigrep(c("DTI"), testnames) ], midnames )
    subcohort[,mcnames]=ANTsRCore::antsrimpute(subcohort[,mcnames])
    subcohort[,'EDUCYRS']=ANTsRCore::antsrimpute(subcohort[,'EDUCYRS'])
    subcohort[,mcnames]=residuals(
        lm( data.matrix( subcohort[,mcnames] ) ~ brainVolume_BL + EDUCYRS + ( Age1 + Age2 ) + SEX, data=subcohort ) )
#    subcohort[,mcnames]=ANTsR::robustMatrixTransform(subcohort[,mcnames])
    subcohort[,mcnames]=scale(ANTsRCore::antsrimpute(subcohort[,mcnames]),T,T)
    subcohortbl[,mcnames]=subcohort[basesel,mcnames]
    subcohorttr=subcohort[trainsel,mcnames]
    myst = trainSubtypeClusterMulti( subcohorttr, mcnames, myclust, 
        desiredk=desiredk )
    mvcl='MVCX'
    subcohort = predictSubtypeClusterMulti( subcohort, mcnames, myst, mvcl, 
        'PATNO', 'imaging_EVENT_ID', 'V0')
    print( table( subcohort$DXSubAsyn, subcohort[,mvcl] ) )
    yth=4.5
    subcohortPD = subcohort[ fs( !is.na(subcohort$AsynStatus) &
        subcohort$DXSubAsyn %in% c('PDSporadicPositive') &
#        subcohort$DXSub  %in% c('PDSporadic') &
        subcohort$yearsbl <= yth),]
    subcohortProdromal = subcohort[ fs( !is.na(subcohort$AsynStatus) &
#        subcohort$DXSubAsyn %in% c('ProdromalSporadicPositive') &
#        subcohort$DXSub %in% c("ProdromalSporadic") &
        subcohort$DX %in% c("Prodromal") &
        subcohort$yearsbl <= yth),]
    subcohortProdromal$CDX = paste0( subcohortProdromal[,mvcl], "_", subcohortProdromal$AsynStatus )
    ...
    print( table( subcohortProdromal$DXSubAsyn, subcohortProdromal[,mvcl] ) )
    print( table( subcohortProdromal$hy, subcohortProdromal[,mvcl] ) )
    for ( mygroup in c( "PD" ) ) {
        if ( mygroup == 'PD' ) thedf=subcohortPD else thedf=subcohortProdromal
...
##########
for ( myg in c( "PD" ) ) {
    gsel = clustdf$group == myg
    print( myg )
    clustdfsub = clustdf[gsel,]
    qvals = p.adjust( as.numeric(clustdfsub$pvalue),'none' )
    table( clustdfsub$method[  qvals < 0.05 ] ) %>% print()
    }
clustdfPD = clustdf
clustlist = as.character( unique( clustdfsub$method[  qvals < 0.05 ]  ) )
clustdf = data.frame( )
for ( myclust in clustlist ) {
    print( paste( "myclust", myclust, 'begin' ) )
    cohsel = clustdata$DX %in% c('PD','Prodromal','CN')
...
#########################################################
for ( cmeth in clustlist ) {
    myg='PD'
    gsel = clustdfPD$group == myg & clustdfPD$method == cmeth
    ...
    cat("\n\n\n")
    print( clustdfsub[ qvals < 0.05 &  clustdfsub$cog %in% clustdfsubPD[  qvalsPD < 0.05, "cog" ],  ] )
}
##########
```
...
```{r mvst2setup,echo=FALSE,eval=TRUE}
######################################################################
for ( m in mymiles ) {
    clin[,m]=as.numeric(clin[,m])
    }
...
clustdata[,mcnames]=residuals(
        lm( data.matrix( clustdata[,mcnames] ) ~ brainVolume_BL + EDUCYRS + ( Age1 + Age2 ) + SEX, data=clustdata ) )
#    clustdata[,mcnames]=ANTsR::robustMatrixTransform(clustdata[,mcnames])
clustdata[,mcnames]=scale(ANTsRCore::antsrimpute(clustdata[,mcnames]),T,T)
osex='SEX'
trainsel = fs( basesel & clustdata$DXSubAsyn %in% c('PDSporadicPositive') )
trainsel = fs( basesel & clustdata$DX %in% c( "PD" ) )#  & subcohort$SEX==0)
trainsel = fs( clustdata$DX != "Prodromal" )
yth=4.5
...
#    clustdata$DXSubAsyn %in% c("ProdromalSporadicPositive") &
...
```{r mvst2Functions,echo=FALSE,eval=TRUE}
...
```{r mvst2prodromaleval,echo=FALSE,eval=TRUE}
mygroup='Prodromal'
clustdf = data.frame( )
