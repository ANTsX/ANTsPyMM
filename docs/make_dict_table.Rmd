...
commcov = " brainVolume_BL + EDUCYRS + T1Hier_resnetGrade + ( Age1 ) + SEX  "
covars0x = commcov
covars0 = paste("testvol_delta ~ (1|PATNO) + (1|SITE)+testvol_BL + ", commcov )
# covars0 = paste("testvol ~ (1|PATNO) + (1|SITE) +  ", commcov )
...
#    studydata[,testnames]=residuals(lm( data.matrix(studydata[,testnames]) ~ studydata$SITE))
    ...
    for ( testvols in testnames ) {
        ...
        studydata$SEX = factor( studydata$SEX )
        toscale = c('snr','psnr','EVR','age_BL','DTI_dti_FD_mean','T1Hier_resnetGrade','DTI_dti_tsnr_b0_mean','testvol_BL','brainVolume_BL','EDUCYRS')
        ...
        bformx=paste( "testvol~(1|PATNO)+(1|SITE)+",covarsx, " + yearsbl ")
        myformx=paste( "testvol~(1|PATNO)+(1|SITE)+",covarsx, " + yearsbl * ( DXSubAsyn )")
        ...
        bmdl = try( lmer( bform,data=studydatasub ) )
        mdl = try( lmer( myform,data=studydatasub ) )
