...
```{r lmerex,echo=FALSE,eval=TRUE,fig.width=12,fig.height=4}
#################
...
cogger='rem'
t0=table( studydata$DXSubAsyn[ studydata$EVENT_ID=='BL'] )
t1=table( studydata$DXSubAsyn[ studydata$EVENT_ID=='V04'] )
nstring='Long N: '
for ( k in 1:length(t1)) nstring=paste0(nstring,names(t1)[k],":",t1[k]," / ")
studydata$yearsbl=studydata$age_at_visit - studydata$age
studydata$yearsbl[ studydata$yearsbl < 0]=0.
...
studydata=studydata[ studydata$yearsbl <= 4.5, ]
...
studydata = fillBaselineColumn( studydata, 
    c('meanthick','meanmd','brainVolume'), 'PATNO', 'yearsbl', 0, 
    fast=T, verbose=F )[[1]]
...
for ( testvols in sample( testnames ) ) {
    ...
    myform="testvol ~ (1|PATNO) + brainVolume_BL + EDUCYRS + noise + snr + psnr + EVR + msk_vol + ol_loop + age + SEX  + mrimfg +  yearsbl * DXSubAsyn"
#    myform="testvol ~ (1|PATNO) + brainVolume + EDUCYRS + age + SEX  + mrimodel +  yearsbl * DXSubAsyn"
    ...
    if ( any( mycoff[,2] < 0.05/length(testnames) ) ) {
        print(testvols)
        print( mycoff )
        nnn = length(unique(studydata$PATNO))
...
#        Sys.sleep(3)
        myform="updrs2_score ~ (1|PATNO) + brainVolume_BL + EDUCYRS + age + SEX + mean_striatum+  (yearsbl * DXSubAsyn * testvol)"
        cogger=sample(coggers,1)
        myform=paste(cogger," ~ (1|PATNO) + brainVolume_BL + EDUCYRS + age + SEX +  (yearsbl * DXSubAsyn * testvol)")
        mdl = lmer( myform,data=studydata[studydata$DXSubAsyn!='CNNegative',] )
        coffpvs = coefficients(summary(mdl))
        coffpvs = coffpvs[grep('testvol',rownames(coffpvs)),5]
        if ( any(coffpvs < 0.005) ) {
            print(cogger)
