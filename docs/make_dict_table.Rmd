...
clustdata = fillBaselineColumn( clustdata,
        ...
        'PATNO', 'imaging_EVENT_ID', 'V0', 
        fast=T, verbose=F )[[1]]
clustdata$milestonesum = rowSums( clustdata[,mymiles] )#############################################################################
...
for ( x in mcnames ) {
    message("SHOULD SEX BE A COVAR GIVEN THAT IT IS COINCIDENT WITH SYMPTOMOLOGY?")
    stcovars = "  ~ T1Hier_resnetGrade + brainVolume_BL + EDUCYRS + ( Age1 + Age2 ) + SEX " 
    stcovars = "  ~ T1Hier_resnetGrade + brainVolume_BL + EDUCYRS + ( Age1 + Age2 ) " 
    stcovars = "  ~ T1Hier_resnetGrade + brainVolume_BL + EDUCYRS + ( Age1 + Age2 ) + mrimfg "
    stcovars = "  ~ SEX + EDUCYRS + ( Age1 + Age2 ) + mrimfg "
    stcovars = "  ~ mrimfg + age_BL + SEX " # age and sex independent
    stcovars = "  ~ mrimfg " # scanner independent
    stcovars = "  ~ mrimfg + age_BL " # age independent
#    stcovars = "  ~ mrimfg + brainVolume_BL + EDUCYRS + ( Age1 + Age2 ) " 
...
desiredk = 5 # based on scree plot etc
nsimlr = 1
minsubcount = 1
######################
```
...
##########
#  "scopa", "stai",
coggersX = c( "gds", "lns", "moca", "NP1APAT", "pigd", 
"quip", "updrs1_score" , "updrs2_score", "updrs3_score", "updrs3_score_on", "updrs_totscore", "updrs_totscore_on", "hvlt_immediaterecall"  )
coggersX2 = c( coggersX )
if ( nsimlr > 0 ) {
    # simlr on mcnames and coggers in training data
    matlist = list(
        scale(antsrimpute(data.matrix( clustdata[clustdata$istrain,mcnames]) )),
        scale(antsrimpute(data.matrix( clustdata[clustdata$istrain,coggersX2] ))) )
...
#    nsimlr = desiredk
    ...
    ebber = 0.99
    nsimlrsearch = 4 # 2:10 
    # max 10 based on looking at the scree plot for cog data
    # 4 has the best p-value and t-stat for cca+pca at default sparseness
    nperms = 20
    # if ( length( nsimlrsearch  ) == 1 ) nperms=3
    quantval = 0.5
    objectiver = 'cca'
    pizzer = c( "positive", "positive" )
    pizzer = c( "positive", "either" )
    pizzer = c( "either", "either" )
    mixer = 'pca'
    nsimlrsearchTvals = data.frame( )
    basesparval = c( 0.5, 0.5 ) 
    # n = 6, n = 4 for 0.8, n=7 for spar 0.8 and LRR
    # n = 3 for spar 0.95 and LRR
    for ( sparsearch in 1 ) {
        set.seed(as.integer( Sys.time() ))
        sparval = rnorm(2,0.75,0.15 )
        if ( sparsearch == 1 ) sparval = basesparval
        if ( sparsearch == 2 ) sparval = c(0.8, 0.8)
        if ( sparsearch == 3 ) sparval = c(0.95, 0.95)
        if ( sparsearch == 4 ) sparval = c(0.,0.)
        ...
        for ( nsimlr in nsimlrsearch ) {
            ...
            for ( nperm in c(1:nperms) ) {
                ...
                for ( jj in 1:length(matlist) ) {
                    myperm = sample( as.integer( 1:nrow(matlist[[jj]])) )
            ...
            nsimlrsearchTvals[nsimlrct,c('sp1','sp2','pos1','pos2', 'elast')]=c(sparval,pizzer, ebber)
            nsimlrsearchTvals[nsimlrct,c('varx','nperms')]=c(refvarxval,nperms)
    ...
    for ( jj in 1:length( simlrX$v ) ) {
        simlrX$v[[jj]]=simlrX$v[[jj]][, simlrpred$uOrder ]
    ...
    projcogs = data.matrix(clustdata[,coggersX2]) %*% simlrX$v[[2]]
...
###################################################
clustlist = c(  
    "hierarchicalCluster", "GMM", "pamCluster", 
    "kmedians", "angle", "ejaccard", "hardcl",  "cckmeans" )
clustlist = c( "GMM", "pamCluster", 
    "kmedians", "angle", "ejaccard", "hardcl",  "cckmeans" )
# clustlist = c( clustlist, mlr_learners$keys("clust") )
# clustlist = clustlist[ !( clustlist %in% c("clust.ff", "clust.agnes","clust.ap","clust.cobweb","clust.dbscan","clust.em","clust.MBatchKMeans","clust.SimpleKMeans","clust.meanshift","clust.xmeans","clust.diana","clust.fanny","clust.hclust","clust.kkmeans" ,     "clust.kmeans"))]
sildf = data.frame()
clustmodels = list()
clustmodelQual = list()
for ( thek in desiredk ) {
...
mypal = scales::hue_pal()(desiredk)
if ( desiredk >= 3 ) mypal[1:3]=c("red","darkgreen","blue")
if ( desiredk == 2 ) mypal=c("red","darkgreen")
...
for ( myclust in clustlistPro ) {
    ...
    subcohortProdromal = predictSubtypeClusterMulti( subcohortProdromal, 
        mcnames, myst, mvcl, 
        'PATNO', 'imaging_EVENT_ID', 'V0')
    if ( length(table( subcohortProdromal[,mvcl] )) > 1 ) {
        ...
        cog2test2 = c( cog2test, 'abeta', 'tau' )
        for ( cog in cog2test ) {
            thedf = subcohortProdromal
            temp = table( thedf$PATNO )
            thedf$yearr =round( thedf$yearsbl)
            tsel = # ( thedf$yearr %in% c(0,1) ) & 
                    thedf$PATNO %in% names(temp)[temp>=minsubcount]
            thedf = thedf[tsel,]
            cogformb=paste( cog , "~", gsub("SEX",osex,commcov), "+(yearsbl)" )
            cogform=paste( cogformb, "+CDX+yearsbl:CDX" )
            ...
            if ( as.numeric(clustdf[nr,c('pvalue')]) < 0.01 & myclust == vizclustmeth ) {
                ...
                myjm = emmeans::joint_tests( cogmdl )
                em1 <- emmeans::emmeans(cogmdl, "CDX", by='yearsbl')
                mypairs1 = pairs(em1)
                print( mypairs1 )
                print( interactions::sim_slopes( cogmdl, 'yearsbl', 'CDX' ) )
...
clustdfPro=clustdf
clustdfPro$pvalue=as.numeric(clustdfPro$pvalue)
#####
```
...
for ( cmeth in clustlistPro ) {
    myg='Prodromal'
    gsel = clustdfPro$group == myg  & clustdfPro$method == cmeth
    clustdfsub = clustdfPro[gsel,]
...
```{r fimp,echo=FALSE,eval=TRUE,fig.width=18,fig.height=9}
myclust='cckmeans'
myclust='kmedians'
...
fimp = featureImportanceForSubtypes(
       vizcohort,
       vizcohort[,mvcl],
       vizcohort[,mcnamesorig],
       associationType = "subtypes2features",
       covariates = "EDUCYRS + ( Age1 + Age2 ) + SEX",
...
# print( summary(lmer( duration_yrs ~  (1|PATNO)+( SEX  ) + MVCX, data=subcohortPD )) )
...
* covariates: manufacturer, age, sex, possibly education

    * probably just age and manufacturer makes most sense
...
## cluster break down with respect to sex


```{r spdsexmvc,echo=FALSE,eval=TRUE}
##########################################
print("PD")
bsel = subcohortPD$imaging_EVENT_ID == 'V0'
temp = subcohortPD[ bsel, ]
print( table( temp[,'SEX'], temp[,mvcl ] ) )
##########################################
print("At risk")
bsel = subcohortProdromal$imaging_EVENT_ID == 'V0'
temp = subcohortProdromal[ bsel, ]
print( table( temp[,'SEX'], temp[,mvcl ] ) )
##########################################
```
...
## cluster break down with respect to subtype


```{r spdpromvc,echo=FALSE,eval=TRUE}
##########################################
##########################################
print("At risk")
bsel = subcohortProdromal$imaging_EVENT_ID == 'V0'
temp = subcohortProdromal[ bsel, ]
print( table( temp[,'subgroup'], temp[,mvcl ] ) )
##########################################
```
...
```{r clusterplot,echo=FALSE,eval=TRUE,fig.width=10,fig.height=8}
library(ggfortify)
pca_res <- prcomp(subcohortPD[,mcnames], scale. = TRUE)
autoplot(pca_res, data = subcohortPD, colour = mvcl, loadings = TRUE)
if ( FALSE ) {
    library(cluster)
    autoplot(pam(subcohortPD[,mcnames], desiredk), frame = TRUE)
    # ggfortify::autoplot( )
    ggplot(subcohortPD, aes(x = PC1, y = PC2, color = MVCX)) +
    geom_point() + theme_minimal() +  stat_ellipse(geom = "polygon",
                aes(fill = MVCX), 
                alpha = 0.25)
    #  stat_ellipse()

    # ggscatter( subcohortPD, mcnames[1], mcnames[2], color=mvcl ) + stat_ellipse()
    #    ggforce::geom_mark_ellipse(aes(fill = mvcl,
    #                        label = mvcl)),
    #                    expand = unit(0.5,"mm"),
    #                   label.buffer = unit(-5, 'mm'))
    #  geom_polygon(data = subcohortPD,
    #               aes(fill = mvcl,
    #                   colour = mvcl),
    #               alpha = 0.3,
    #               show.legend = FALSE)
