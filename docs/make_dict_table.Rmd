...
covars0x = " EDUCYRS + snr + mrimfg + age_BL + SEX "
...
commonsel = fs(clin$T1Hier_resnetGrade >= 1.05 & clin$yearsbl <= 2.5 &
...
for (  subtype in subtypes ) {
    ...
    for ( testvols in ( testnames ) ) {
        ...
        myformx=paste( covarsx, " + ( DXSubAsyn )")
        mdl = try( lmer( myform,data=studydata ) )
        studydatax=studydata[studydata$imaging_EVENT_ID=='V0',]
        studydatax$EDUCYRS=as.numeric(studydatax$EDUCYRS)
        bmdlx = lm( paste("testvol~",covarsx),data=studydatax )
        mdlx = lm( paste("testvol~",myformx),data=studydatax )
        if ( !is(mdl, "try-error") ) {
            mycoffx = coefficients(summary(mdlx))[-c(1:2),-1]
            mycoff = coefficients(summary(mdl))[-c(1:2),-c(1:2)]
            colnames(mycoffx)[1]='df'
            mycoffx[,'df']=summary(mdlx)$df[2]
            # mycoffTau = mycoff[multigrep(c(subtype,"ptau"),rownames(mycoff),intersect=F),]
            mycoff = rbind(
...
makethetable <- function( ss ) {
    pvaluedfsub = pvaluedf[ pvaluedf$subtype == ss,c("anat","anova", "Neg","Pos","Y.Neg","Y.Pos") ]
    sel=p.adjust( pvaluedfsub[,'anova'], corrmeth ) <= 0.05
    pvaluedfsub=pvaluedfsub[sel,]
    for ( x in c( "anova", "Neg","Pos","Y.Neg","Y.Pos" ) ) {
