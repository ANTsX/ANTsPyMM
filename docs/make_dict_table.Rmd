...
```{r lmerex,echo=FALSE,eval=FALSE}
library(lmerTest)
...
testvols = getNamesFromDataframe( c("T1Hier","thk","alecmtl"), studydata )
studydata$testvol = rowMeans( studydata[,testvols[c(1,2)]])
studydata$EDUCYRS = as.numeric( studydata$EDUCYRS )
mdl = lmer( testvol ~ (1|PATNO) + EDUCYRS + age + SEX  + yearsbl * DXSubAsyn,data=studydata )
