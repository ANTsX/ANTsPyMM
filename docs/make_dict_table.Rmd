...
```{r joiner2,eval=FALSE,echo=FALSE}
...
t1names=c( 
        ...
        getNamesFromDataframe( "T1Hier_thk", dd, 
            exclusions=c("_BL","_delta","evratio","Grade","mhdist","RandB","outli","templateL1",'filename','u_hier_id') ) )
dtinames=getNamesFromDataframe( "DTI"  , dd, exclusions=c("_BL","_delta","FD","motion","SNR","evr","ssnr","dvars","tsnr","volume",'filename','u_hier_id','cnx') )
    
rsfnames=getNamesFromDataframe( "rsfMRI"  , dd, exclusions=c("_BL","_delta","cnx","FD","motion","SNR","evr","ssnr","dvars","tsnr","volume","_alff",'filename','u_hier_id') )
...
ssel = fs( clin$AsynStatus %in% c('Positive','Negative') ) & 
#    fs( recentdates ) &
#    fs( clin$imagedatediff <= 1 ) &
    fs( clin$DX %in% c("PD","CN") ) & 
    fs( clin$DXSubAsyn %in% dxs ) & 
    !is.na(clin$T1Hier_resnetGrade) &
#    fs( !clin$mrimodel %in% c("SIGNA") ) &
#    fs( clin$mrimfg %in% c( "Siemens") ) & 
#    fs( clin$mrimfg %in% c( "Siemens", "Phillips","GE", "Unk" ) ) & 
    fs(clin$T1Hier_resnetGrade >= 1.05)
table( ssel )
studydata = clin[ssel,]
coggers = c( "ess", "gds","lexical",'lns','moca','MODBNT','NP1APAT','NP1ANXS',
    "mean_striatum",'NP1DPRS', 'pigd', 'rem', 'quip','scopa','stai','VLTANIM',
    'nfl_serum', #upsit
    getNamesFromDataframe("score",studydata), 
    getNamesFromDataframe("HVLT",studydata), 
    getNamesFromDataframe("hvlt_",studydata), 
    getNamesFromDataframe("pm_",studydata) )
for ( cog in coggers )
    studydata[,cog]=as.numeric(studydata[,cog])
mymiles = c("pm_adl_any","pm_fd_any","pm_auto_any", "pm_cog_any",           "pm_mc_any" ,           "pm_wb_any")
studydata$milestonesum = rowSums( studydata[,mymiles] )
# studydata = highestQualityRepeat( studydata, 'PATNO', 'yearsbl', 'T1Hier_resnetGrade')
# studydata = highestQualityRepeat( studydata, 'PATNO', 'u_hier_id', 'T1Hier_resnetGrade')
...
studydata$brainVolume = rowSums( studydata[,getNamesFromDataframe( c("T1Hier","vol","hemis"), studydata )])
studydata$brainVolume = studydata$brainVolume/mean(studydata$brainVolume,na.rm=T)
studydata=studydata[ studydata$yearsbl <= 40.5, ]
...
studydata = fillBaselineColumn( studydata,
    ...
    fast=T, verbose=F )[[1]]
# studydata$testvol = rowMeans( studydata[,testvols[]])
...
#######################################
for ( testvols in sample( testnames ) ) {
#    testvols=sample( testnames, 1 )
    studydata$testvol = studydata[ ,testvols]
    studydata$testvol = psych::winsor(studydata$testvol,tr=0.01)
    studydata$EDUCYRS = as.numeric( studydata$EDUCYRS )
    myform="testvol ~ (1|PATNO) + brainVolume_BL + EDUCYRS + snr + psnr + EVR + age_BL + SEX + yearsbl * DXSubAsyn"
    mdl = lmer( myform,data=studydata )
#    summary(mdl)
#    testvols
    mycoff = coefficients(summary(mdl))[-c(1:2),-c(1:3)]
    mycoff = mycoff[grep(subtype,rownames(mycoff)),]
    if ( any( mycoff[,2] < 0.05/length(testnames) ) ) {
        print(testvols)
        print( mycoff )
...
#        print( visreg::visreg( mdl, 'yearsbl', by='DXSubAsyn',gg=TRUE) + ggtitle(testvols) )
        (prplot( mdl,  'yearsbl', 'DXSubAsyn', color='DXSubAsyn', addpoints=1 )+ ggtitle(paste(testvols,"\n",nstring))) %>% print()
#        Sys.sleep(3)
        myform="updrs2_score ~ (1|PATNO) + brainVolume_BL + EDUCYRS + age_BL + SEX + mean_striatum+  (yearsbl * DXSubAsyn * testvol)"
        if ( FALSE )
        for ( cogger in coggers ) {
            bform=paste(cogger," ~ (1|PATNO) + brainVolume_BL + EDUCYRS + age_BL + SEX +  (yearsbl * DXSubAsyn )")
            mform=paste( bform , "*testvol")
            ss = fs(studydata$DXSubAsyn!='CNNegative' & 
                !is.na(studydata[,testvols])& 
                !is.na(studydata[,cogger])& 
                !is.na(studydata[,'brainVolume_BL'])& 
                !is.na(studydata[,'age'])& 
                !is.na(studydata[,'yearsbl'])& 
                !is.na(studydata[,'EDUCYRS'])& 
                !is.na(studydata[,'SEX']))
            datob = studydata[ss,]
            mdl = lmer( mform,data=datob )
            bmdl = lmer( bform,data=datob[names(predict(mdl)),] )
            mdl = lmer( mform,data=datob[names(predict(mdl)),] )
            coffpvs = coefficients(summary(mdl))
            coffpvs = coffpvs[grep('testvol',rownames(coffpvs)),5]
            if ( anova(bmdl,mdl)$Pr[2] < 0.001 ) {
                print(cogger)
                print( coefficients(summary(mdl))[-c(1:3),-c(1:3)] )
                print( threewayinteraction( studydata,  'yearsbl', cogger,  'DXSubAsyn', anat=testvols ) )
                }
            }
        }
    }
