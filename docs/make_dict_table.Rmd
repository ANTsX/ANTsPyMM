...
baseimpute <-function( mydf, varofinterest ) {
    ...
    mydf[isna,vobl]=mean(mydf[!isna & isbl,vobl],na.rm=T)
    disna=is.na( mydf[,vodl] )
    mydf[disna,vodl]=mydf[disna,varofinterest]-mydf[disna,vobl]
    return( mydf )
    }

visitimpute <-function( mydf, varofinterest ) {
    yearr = round( mydf$yearsbl )
    vobl = paste0(varofinterest,"_BL")
    vodl = paste0(varofinterest,"_delta")
    ...
    for ( v in c(0,1,2,4) ) {
        for ( u in udx ) {
            ...
            mydf[isna,varofinterest]=mean(mydf[!isna & isviz,varofinterest],na.rm=T)
            }
        }
...
t1names = t1names[ 
    multigrep(c( "fusiform", "nbm", "supra","temporal","parietal","central","snc","_pu_","_ca_","_snr","_gpi","_gpe"), t1names ) ]
dtinames=c(
...
commcov = " brainVolume_BL + EDUCYRS + snr + EVR + ( Age1 + Age2 ) * SEX  "
covars0x = commcov
covars0 = paste("testvol ~ (1|PATNO) +  ", commcov )
covars0 = paste("testvol_delta ~ (1|PATNO) + (1|SITE)+testvol_BL + ", commcov )
...
#    studydata[,testnames]=residuals(lm( data.matrix(studydata[,testnames]) ~ studydata$SITE))
    ...
    studydata = fillBaselineColumn( studydata,
        c('meanthick','meanmd','brainVolume',testnames), 
        'PATNO', 'imaging_EVENT_ID', 'V0', 
        fast=T, verbose=F )[[1]]
    for ( x in  c('meanthick','meanmd','brainVolume',testnames) ) {
        studydata=visitimpute( studydata, x )
    }
##        studydata=baseimpute( studydata, 'testvol' )
    # testnames='DTI_mean_md_Asym_rostral_middle_frontal'
    for ( testvols in testnames ) {
        ...
        bformx=paste( "testvol~(1|PATNO)+(1|SITE)+",covarsx, " ")
        myformx=paste( "testvol~(1|PATNO)+(1|SITE)+",covarsx, " +  ( DXSubAsyn )")
        studydata$EDUCYRS=as.numeric(studydata$EDUCYRS)
        studydata$age_BL=ANTsRCore::antsrimpute(as.numeric(studydata$age_BL))
        ...
        bmdlx = lmer( bformx,data=studydatasub )
        mdlx = lmer( myformx,data=studydatasub )
        if ( FALSE ) {
        ...
        if ( !is( mdl, "try-error" ) ) {
            ...
            if (  pvaluedf[ct,'anova'] < qthresh | pvaluedf[ct,'anovax'] < qthresh  )
                ...
                mypal = 'lancet'
                (prplot( mdlx, 'DXSubAsyn',color='DXSubAsyn', addpoints=1, palette=mypal )+ ggtitle(paste(testvols,pvformattedx,"\n",unlist(strsplit(nstring,"\n"))[1]))) %>% print()
                ...
                if ( TRUE ) {
                    ...
                    print( plotSubtypeChange( studydatasub[tsel,],
                        ...
                        scale_colour_manual(values = manualColors ) )
                    ggscatter( studydata, 'yearsbl', "testvol", facet.by='DXSubAsyn', color='DXSubAsyn', add='reg.line', conf.int=T, cor.coef=T, parse=T, palette=manualColors ) %>% print()
                    }
                }
            }
        }
