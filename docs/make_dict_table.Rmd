...
#    nsimlr = desiredk
    ...
    myjr = TRUE
    prescaling = c( 'robust', 'norm'  )
    prescaling = c( 'centerAndScale', 'norm' )
    optimus = 'lineSearch'
    maxits = 100
    ebber = 0.5
    nsimlrsearch = 2 # 2:8 # max 10 based on looking at the scree plot for cog data
    nperms = 100
    # if ( length( nsimlrsearch  ) == 1 ) nperms=3
    quantval = 0.99
    objectiver = 'lowRankRegression'
...
```{r clusterplot,echo=FALSE,eval=TRUE}
# library(cluster)
# autoplot(clara(subcohortPD[,mcnames], desiredk), frame = TRUE)
# ggfortify::autoplot( )
ggscatter( subcohortPD, mcnames[1], mcnames[2], color=mvcl )
