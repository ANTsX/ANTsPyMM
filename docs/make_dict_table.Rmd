...
countdtinz=rep(NA,length(testnames))
for ( x in 1:length(testnames) ) {
        countdtinz[x]=sum( !is.na(clustdata[,testnames[x]]) & clustdata[,testnames[x]] > 0)
    }
testnames=testnames[ countdtinz > 100 ]
#    testnames = testnames[ -grep("Asym",testnames)]
...
# cohsel = clustdata$DXSubAsyn %in% c('PDSporadicPositive')
subcohort = clustdata[ cohsel,]
basesel = fs(subcohort$imaging_EVENT_ID == 'V0')
trainsel = fs( basesel & subcohort$DX %in%  c('Prodromal','CN' ) )
trainsel = fs( basesel & subcohort$hy_BL %in%  c(0) )
...
mcnames = testnames[ -multigrep(c("DTI"), testnames) ]
...
myst = trainSubtypeClusterMulti( subcohorttr, mcnames, 'angle', desiredk=3 )
mvcl='MVCX'
...
subcohortProdromal = subcohort[ fs( !is.na(subcohort$AsynStatus) &
    subcohort$DX %in% c("PD") & subcohort$yearsbl <= 4.5),]
subcohortProdromal$CDX = paste0(subcohortProdromal$AsynStatus,subcohortProdromal[,mvcl])
#    subcohort$DXSub %in% c("PDSporadic") & subcohort$yearsbl <= 4.5,]
# subcohortProdromal = subcohort[ subcohort$hy_BL > 0,]
# subcohortProdromal = subcohort[ subcohort$DX == 'PD' & subcohort$yearsbl <= 4.5,]
# subcohortProdromal = subcohort[ subcohort$yearsbl <= 2.5,]
...
for ( cog in coggers[ !( coggers %in% c('mean_striatum','updrs4_score') ) ] ) {
# for ( cog in coggers ) {
    print(paste("cog",cog))
    temp = table( subcohortProdromal$PATNO )
    subcohortProdromal$yearr =round( subcohortProdromal$yearsbl)
    tsel = ( subcohortProdromal$yearr %in% c(0,1,2,4) ) & 
                    subcohortProdromal$PATNO %in% names(temp)[temp>=1]
    subcohortProdromal = subcohortProdromal[tsel,]
    cogform=paste( cog , "~", commcov, "+(yearsbl)*CDX" )
    cogmdl = lmer( cogform, data=subcohortProdromal )
    cogcoff=coefficients(summary(cogmdl))
    ...
    if ( any( cogcoff[,"Pr(>|t|)"] < 0.001 ) ) {
        ...
        cogpvform = insight::format_p( 
            min(cogcoff[,"Pr(>|t|)"]),
            stars=TRUE, digits=4 )
        print( plotSubtypeChange( subcohortProdromal,
                        idvar='PATNO', 'CDX',
                        measurement=cog,
                        vizname='yearr', 'se', xlab='yr from v0' ) + 
                        ggtitle(paste(cog," : raw"))  )
        subcohortProdromal$temp = as.character( subcohortProdromal[,'DXSub'])
        subcohortg=subcohortProdromal[!is.na(subcohortProdromal$temp),]
        ggscatter( subcohortg, 'yearsbl', cog, facet.by='CDX', color='temp', add='reg.line', conf.int=T, cor.coef=T, parse=T ) %>% print()
