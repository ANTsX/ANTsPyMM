...
if ( nsimlr > 0 ) {
    ...
    mysvdd
    nsimlr = 4 # based on PCA plot
    # which.min( abs(0.8 - (mysvdd/max(mysvdd)) ))
#    nsimlr = 2
#    nsimlr = desiredk
    ...
    initu = initializeSimlr(
        ...
        addNoise = 0 )
    # if ( ! exists( "simlrX" ) )
    simlrX = simlr( matlist, regs, iterations=100, verbose=TRUE,
            randomSeed = 0,
            mixAlg='pca',
            energyType='cca',
#            scale = 'robust',
#            sparsenessQuantiles=c(0.95,0.95),
            sparsenessQuantiles=c(0.8,0.8),
            positivities = c('positive','either'),
            constraint = 'none',
            expBeta = 0.90,
            initialUMatrix=initu )
    print( predictSimlr( matlist, simlrX )$varx )
    mcnamesorig = mcnames
...
```{r gap,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE,cache=FALSE}
nboot = 200
...
```{r nbclust,echo=FALSE,eval=TRUE,cache=FALSE}
...
```{r clustree,echo=FALSE,eval=TRUE,cache=FALSE}
tmp <- NULL
for (k in 1:11){
...
```{r clvalid,echo=FALSE,eval=TRUE,cache=FALSE}
...
```{r pamk,echo=FALSE,eval=TRUE,cache=FALSE}
library(fpc)
...
```{r Calinski,echo=FALSE,eval=TRUE,cache=FALSE}
require(vegan)
...
```{r bic,echo=FALSE,eval=TRUE,cache=FALSE}
library(mclust)
...
```{r aff,echo=FALSE,eval=TRUE,cache=FALSE}
library(apcluster)
...
clustlistPro = as.character( unique( c( "cckmeans", "kmedians",clustdfPD$method[  qvals <= 0.05 ]  ) ))
...
```{r vizinpd,echo=FALSE,eval=TRUE,fig.width=9,fig.height=3}
...
for ( zz in cog2test ) {
    gg1=ggviolin( subcohortPD[basesel,], mvcl, zz)+ggtitle(paste("Baseline",zz))
    follsel = round(subcohortPD$yearsbl)==4
    gg2=ggviolin( subcohortPD[follsel,], mvcl, zz)+ggtitle(paste("Year 4",zz))
    print( grid.arrange( grobs=list(gg1,gg2),nrow=1))
    }
