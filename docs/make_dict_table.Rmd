...
#                    ggscatter( studydata, 'yearsbl', "brainVolume", facet.by='DXSubAsyn', color='DXSubAsyn', add='reg.line', conf.int=T, cor.coef=T, parse=T, palette=manualColors ) %>% print()
                    ...
                    testcog=TRUE
                    if ( testcog ) {
                        subcohort = studydata[ studydata$DXSubAsyn != 'CNNegative',]
                        subcohortbl = studydata[ studydata$DXSubAsyn !=     
                            'CNNegative' &
                            studydata$imaging_EVENT_ID == 'V0', ]
                        ...
                        subcohort = predictSubtypeUni( subcohort, myst, 'PATNO', 
                            'imaging_EVENT_ID', 'V0')
                        for ( cog in coggers ) {
                            cogform=paste( cog , "~", commcov, "+subtype*yearsbl" )
                            mdl = try( lmer( cogform, data=subcohort ) )
                            cogcoff=coefficients(summary(mdl))
                            ...
                            if ( any( cogcoff[,"Pr(>|t|)"] < 0.001 ) ) {
                                ...
                                cogpvform = insight::format_p( 
                                    min(cogcoff[,"Pr(>|t|)"]),
                                    stars=TRUE, digits=4 )
                                (prplot( mdl,'yearsbl',  'subtype',color='subtype', addpoints=1, palette=mypal )+ ggtitle(paste(testvols,cog,"\n",cogpvform))) %>% print()
...
## test the temporoparietal junction with cognition


```{r temporoparietal,echo=FALSE}
