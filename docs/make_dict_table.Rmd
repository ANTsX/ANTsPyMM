...
t1names=c( 
        ...
        getNamesFromDataframe( c("T1Hier_vol",'brainstem'), dd, 
...
t1names = unique( t1names[ 
    multigrep(c( "fusiform", "nbm", "supra","temporal","parietal","central","snc","_pu_","_ca_","_snr","_gpi","_gpe","cerebellum","pons","medulla","midbrain"), t1names ) ] )
dtinames=c(
...
for (  subtype in subtypes ) {
    ...
    studydata = clin[ssel,]
    print( table(round(studydatasub$yearsbl), studydatasub$DXSubAsyn) )
...
trainsel = fs( basesel & subcohort$DX == 'PD' )
subcohortbl = subcohort[ basesel, ]
mcnames = testnames[ -grep("DTI",testnames) ]
...
myst = trainSubtypeClusterMulti( subcohorttr, mcnames, 'jaccard', 6 )
mvcl='MVCX'
subcohort = predictSubtypeClusterMulti( subcohort, mcnames, myst, mvcl, 
    'PATNO', 'imaging_EVENT_ID', 'V0')
print( table( subcohort$DXSubAsyn, subcohort[,mvcl] ))
subcohortProdromal = subcohort[ subcohort$DX != 'PD' & subcohort$yearsbl <= 2.5,]
...
for ( cog in coggers[ !( coggers %in% c('mean_striatum','updrs4_score') ) ] ) {
    cogform=paste( cog , "~", commcov, "+(yearsbl)*",mvcl )
    ...
    if ( any( cogcoff[,"Pr(>|t|)"] < 0.001 ) ) {
        ...
        cogpvform = insight::format_p( 
            min(cogcoff[,"Pr(>|t|)"]),
            stars=TRUE, digits=4 )
        temp = table( subcohort$PATNO )
        subcohort$yearr =round( subcohort$yearsbl)
        tsel = ( subcohort$yearr %in% c(0,1,2) ) & 
                    subcohort$PATNO %in% names(temp)[temp>=2]
        print( plotSubtypeChange( subcohort[tsel,],
                        idvar='PATNO', mvcl,
                        measurement=cog,
                        vizname='yearr', 'se', xlab='yr from v0' ) + 
                        ggtitle(paste(cog," : raw"))  )
#        (prplot( cogmdl,'yearsbl', mvcl, color=mvcl, 
