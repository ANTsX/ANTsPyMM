...
# convert shortbirtdate to NRG format
...
for ( k in 1:nrow( clin ) ) {
    ...
    isel = which( ee$PATNO == pat )
    iage = ee$ageatimaging[ isel ]
    minagediff = min( abs( iage - patage ), na.rm=T)
    if ( minagediff < yearthresh ) {
...
# find the best demographic data for each imaging time point
```
