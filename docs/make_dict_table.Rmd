...
dtinames=unique(
    getNamesFromDataframe( c("DTI","mean_md"), dd, exclusions=c("_BL","_delta","FD","motion","SNR","evr","ssnr","dvars","tsnr","volume",'filename','u_hier_id','cnx','snseg','tcit') )  )
...
covars = "testvol ~ (1|PATNO) + brainVolume_BL + EDUCYRS + snr + T1Hier_resnetGrade + age_BL + SEX "
covars = "testvol ~ (1|PATNO) + brainVolume_BL + EDUCYRS + snr + psnr + EVR + T1Hier_resnetGrade + mrimfg + age_BL + SEX "
covars = "testvol ~ (1|PATNO) + brainVolume_BL + EDUCYRS + mrimfg + snr + T1Hier_resnetGrade + age_BL + SEX "
covars = "testvol ~ (1|PATNO) + brainVolume_BL + EDUCYRS + snr + mrimfg + age_BL + SEX "
dticovars="+DTI_dti_FD_mean+DTI_dti_tsnr_b0_mean"
dticovars="+DTI_dti_FD_mean"
#################################################
```

```{r lmerextab,echo=FALSE,eval=TRUE,fig.width=12,fig.height=5,message=FALSE,warning=FALSE}
###################################################
winsval=0.005
qthresh=1e-4
...
for (  subtype in subtypes ) {
    ...
    studydata=studydata[ studydata$yearsbl <= 14.5, ]
...
for ( subtype in subtypes[1:2] ) {
    ...
    sel=p.adjust( pvaluedfsub[,'anova'], 'bonf' ) <= 0.05
...
```{r printlrrk2,echo=FALSE,eval=TRUE}
makethetable("LRRK2")
```

## sporadic results table

```{r printsporadic,echo=FALSE,eval=TRUE}
makethetable('Sporadic')
```


```{r lmerexallgroups,echo=FALSE,eval=TRUE,fig.width=12,fig.height=8,message=FALSE,warning=FALSE}
###################################################
winsval=00.00
qthresh=1.e-4
dxstbl = table( clin$DXSubAsyn  )
dxs = names(dxstbl[dxstbl>6])
dxs = dxs[ -2 ]
ct = 0
ssel = fs( clin$DXSubAsyn %in% dxs ) & 
        !is.na(clin$T1Hier_resnetGrade) &
...
##########################################################
for ( x in names(pvaluedfG)[1:12] ) {
    print( x  )
    sel=p.adjust( pvaluedfG[,x], 'bonf' ) <= 0.05
